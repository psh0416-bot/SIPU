...
run_dataset_suite_unknown() {
  local seed="$1"
  local dataset="$2"
  local extra_args
  extra_args="$(dataset_extra_args "$dataset")"

  run_main_oom_ok --data "$dataset" --model mlp --loss ure --study "$STUDY" --seed "$seed" $extra_args
  run_main_oom_ok --data "$dataset" --model gat --loss ure --study "$STUDY" --seed "$seed" $extra_args
  run_main_oom_ok --data "$dataset" --model mlp --loss nre --study "$STUDY" --seed "$seed" $extra_args
  run_main_oom_ok --data "$dataset" --model gat --loss nre --study "$STUDY" --seed "$seed" $extra_args
  run_main_oom_ok --data "$dataset" --model pulp --loss distpu --study "$STUDY" --seed "$seed" $extra_args
  run_main_oom_ok --data "$dataset" --model lsdan --loss nre --study "$STUDY" --seed "$seed" $extra_args
}
...
do
  run_dataset_suite_unknown "$i" chameleon-filtered
  run_dataset_suite_unknown "$i" twitch-en
  run_dataset_suite_unknown "$i" facebook
  run_dataset_suite_unknown "$i" pubmed
  run_dataset_suite_unknown "$i" actor
  run_dataset_suite_unknown "$i" roman-empire
  run_dataset_suite_unknown "$i" amazon-computers
  run_dataset_suite_unknown "$i" amazon-photo
done
